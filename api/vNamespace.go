...
func ListNamespace(g *gin.Context) {
	...
	case "true", "false", "1", "0", "":
		...
		g.JSON(http.StatusOK, general.ResponseSuccessful("查询成功", list))

	default:
		g.JSON(http.StatusBadRequest, general.ResponseFailure("参数错误", errors.New("is_active的值只能为布尔值true、false")))
	}
}
...
func PostNamespace(g *gin.Context) {
	...
	if err != nil {
		g.JSON(http.StatusBadRequest, general.ResponseFailure("命名空间已存在，不能重复创建", err))
	} else {
		g.JSON(http.StatusOK, general.ResponseSuccessful("命名空间创建成功", &namespace))
	}
}
...
func GetNamespace(g *gin.Context) {
	...
	if err != nil {
		g.JSON(http.StatusBadRequest, general.ResponseFailure("参数错误", err))
	} else {
		g.JSON(http.StatusOK, general.ResponseSuccessful("查询成功", result))
	}
}
...
func PutNamespace(g *gin.Context) {
	...
	if err != nil {
		g.JSON(http.StatusBadRequest, general.ResponseFailure("namespace名称不能重复", err))
	} else {
		g.JSON(http.StatusOK, general.ResponseSuccessful("修改成功", result))
	}
}
...
func DeleteNamespace(g *gin.Context) {
	...
	if err != nil {
		...
		g.JSON(http.StatusOK, general.ResponseSuccessful("删除操作执行成功", fmt.Sprintf("受影响的行数：%v", num)))
	}
}
