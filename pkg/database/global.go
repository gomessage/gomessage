...
type databaseClientList struct {
	DefaultClient *gorm.DB //通用客户端，给它赋值不同的数据库对象（比如MySQL或者SQLite），可以最小限度的修改全局其它地方的代码。
...
func (d *databaseClientList) Init(databaseType string) {
	...
	case "mysql":
		DB = &databaseClientList{
			DefaultClient: mysqlClient(), //通用客户端，给它赋值不同的数据库对象（比如MySQL或者SQLite），可以不用修改全局其它地方的代码
			Mysql:         mysqlClient(), //Mysql专用客户端
		}

	case "sqlite", "sqlite3":
		DB = &databaseClientList{
			DefaultClient: sqlite3Client(), //通用客户端，给它赋值不同的数据库对象（比如MySQL或者SQLite），可以不用修改全局其它地方的代码
