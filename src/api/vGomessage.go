...
import (
	...
	"gomessage/src/services/core/v1"
	v32 "gomessage/src/services/core/v3"
	"gomessage/src/services/hijacking"
	"io"
	"net/http"
	"time"
)
...
func GoMessageByTransport(g *gin.Context) {
	...
	loggers2.PushLogger.WithFields(logrus.Fields{
...
func GoMessageByGet(g *gin.Context) {
	...
	loggers2.DefaultLogger.Info("当前命名空间为：", namespace)

	result, err := models2.GetNamespaceByName(namespace)
	if err != nil {
		g.JSON(http.StatusBadRequest, utils2.ResponseFailure("namespace不存在", err))
	} else {
		g.JSON(http.StatusOK, utils2.ResponseSuccessful("namespace ready", result))
	}
}
