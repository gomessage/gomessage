...
func RendersContentData(requestByte []byte, varMap []map[string]string, msgTemplate string) []string {
	analysisDataList := AnalysisDataV2(varMap, requestByte)  //得到变量映射
	return v1.CompleteMessage(msgTemplate, analysisDataList) //得到内容体
}

func AnalysisDataV2(keyValueList []map[string]string, requestData []byte) []map[string]string {
	/*
